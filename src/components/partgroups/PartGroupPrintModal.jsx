...
import {
  GROUP_REPORT_CONFIGS,
  PRICING_MODES,
  PRICING_MODE_LABELS,
  loadReportOptions,
  savePrintOptions,
  loadSavedPrintOptions,
} from "@/components/parts/print/sharedPrintConfig";

const STORAGE_KEY = "ak_group_print_options";

/**
 * Print options modal for Parts Groups.
 * Uses canonical pricingMode control — no separate Cost/Retail toggles.
 * Default: RETAIL_ONLY (client-safe).
 */
export default function PartGroupPrintModal({ onClose, onPrint }) {
  const [reportType, setReportType] = useState("summary");
  const [opts, setOpts] = useState(() =>
    loadReportOptions(STORAGE_KEY, "summary", GROUP_REPORT_CONFIGS)
  );
  ...
  const handleTypeChange = (type) => {
    setReportType(type);
    setOpts(loadReportOptions(STORAGE_KEY, type, GROUP_REPORT_CONFIGS));
  };
  ...
  const handlePrint = () => {
    ...
    // Save clean — only persist pricingMode, never legacy includeCost/includeRetail
    const toSave = { ...opts };
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-2 pb-2 border-b border-gray-800">
          ...
          <Select
            value={opts.pricingMode || PRICING_MODES.RETAIL_ONLY}
