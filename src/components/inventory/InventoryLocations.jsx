...
import { 
  Search, MapPin, ChevronRight, ChevronDown, Package, LayoutGrid, List, Star, X
} from "lucide-react";
...
export default function InventoryLocations({ onPartClick, urlLocationId }) {
  ...
  const getDescendants = useCallback((locationId) => {
    ...
    locations.filter(loc => loc.parent_id === locationId).forEach(child => { descendants.push(...getDescendants(child.id)); });
    return descendants;
  }, [locations]);
  ...
  const locationPartCounts = useMemo(() => {
    ...
    locations.forEach(loc => {
      ...
      inventoryItems.forEach(item => {
        if (locIds.includes(item.location_id) && (item.quantity_on_hand || 0) > 0) partsHere.add(item.part_id);
      });
      counts[loc.id] = partsHere.size;
    });
    const unassigned = new Set();
    inventoryItems.forEach(item => { if (!item.location_id && (item.quantity_on_hand || 0) > 0) unassigned.add(item.part_id); });
  ...
  const searchResults = useMemo(() => {
    ...
    parts.forEach(p => {
      ...
      if (score > 0) partResults.push({ part: p, score });
    });
    ...
    containers.forEach(c => {
      ...
      if (score > 0) containerResults.push({ container: c, score });
    });
    ...
    locations.forEach(loc => {
      ...
      if (score > 0) locationResults.push({ loc, score });
    });

    return {
      parts: partResults.sort((a, b) => b.score - a.score).slice(0, 10).map(s => s.part),
      containers: containerResults.sort((a, b) => b.score - a.score).slice(0, 10).map(s => s.container),
      locations: locationResults.sort((a, b) => b.score - a.score).slice(0, 15).map(s => s.loc),
      total: partResults.length + containerResults.length + locationResults.length,
    };
  }, [searchTerm, hasSearch, parts, containers, locations, inventoryItems, projects]);

  // Part search within selected location (filters content pane)
  const filteredParts = useMemo(() => {
    if (!searchTerm) return partsAtSelectedLocation;
    const term = searchTerm.toLowerCase();
    return partsAtSelectedLocation.filter(part =>
      part.part_name?.toLowerCase().includes(term) || part.vendor_part_number?.toLowerCase().includes(term)
    );
  ...
  const clearSearch = () => { setSearchTerm(''); setSearchFocused(false); };
    ...
    return (
      <div key={location.id}>
        ...
        >
          ...
          <span className={cn("flex-1 text-sm font-medium truncate", isSelected && "font-semibold")} style={{ color: isSelected ? (location.color || '#EF4444') : undefined }} title={location.location_area}>
  ...
  return (
    <>
      <div className="flex flex-col bg-black/20 rounded-lg border border-red-900/30 md:h-[calc(100vh-8rem)] md:overflow-hidden">
        ...
        <div className="p-3 bg-black/40 backdrop-blur-xl border-b border-red-900/30">
          <div className="relative">
            ...
            <Input
              placeholder="Find a part, container, or location…"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              onFocus={() => setSearchFocused(true)}
              className="pl-11 pr-10 bg-gray-900/50 border-gray-700 text-white h-11 text-base"
            />
            {searchTerm && (
          ...
          {/* Subtitle — only when not searching */}
          {!hasSearch && (
            <div className="flex items-center justify-between mt-2">
              <p className="text-xs text-gray-500">
                {selectedContainer
                  ? selectedContainer.name
                  : `${filteredParts.length} parts · ${getSelectedLocationName()}`}
              </p>
              <div className="flex items-center gap-1 bg-black/40 border border-gray-700 rounded-lg p-0.5">
                <Button size="sm" variant={viewMode === 'list' ? 'default' : 'ghost'} onClick={() => setViewMode('list')} className={cn("h-6 px-1.5", viewMode === 'list' ? 'bg-red-600 text-white' : 'text-gray-400 hover:text-white')}>
                  <List className="w-3.5 h-3.5" />
                </Button>
                <Button size="sm" variant={viewMode === 'cards' ? 'default' : 'ghost'} onClick={() => setViewMode('cards')} className={cn("h-6 px-1.5", viewMode === 'cards' ? 'bg-red-600 text-white' : 'text-gray-400 hover:text-white')}>
        ...
        {/* Search results overlay — takes over the full content area */}
        {showSearchOverlay ? (
          <div className="flex-1 overflow-y-auto p-3 space-y-4">
            {/* Parts first — most common search intent */}
                  ...
                  {searchResults.parts.map(p => {
                    ...
                    return (
                      <button key={p.id} onClick={() => { onPartClick?.(p); clearSearch(); }}
                        className="w-full flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-gray-800/50 text-left border border-transparent hover:border-gray-700 transition-colors">
                        ...
                        <div className="text-right shrink-0">
                          <div className="text-sm text-white font-semibold">{totalQty}</div>
                  ...
                  {searchResults.containers.map(c => {
                    ...
                    return (
                      <button key={c.id} onClick={() => { setSelectedContainer(c); clearSearch(); }}
                        className="w-full flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-gray-800/50 text-left border border-transparent hover:border-gray-700 transition-colors">
                        {c.photo ? (
                          <img src={c.photo} alt={c.name} className="w-10 h-10 rounded object-cover border border-gray-700 shrink-0" loading="lazy" />
                        ) : (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {c.short_code && <span className="text-xs font-mono font-bold text-gray-400">{c.short_code}</span>}
                          </div>
                          <div className="text-xs text-gray-500 truncate mt-0.5">
                        ...
                        <div className="text-right shrink-0">
                          <div className="text-sm text-white font-semibold">{cItemCount}</div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {searchResults.locations.map(loc => {
                    ...
                    const partCount = locationPartCounts[loc.id] || 0;
                    return (
                      <button key={loc.id} onClick={() => { handleLocationSelect(loc.id); clearSearch(); }}
                        className="w-full flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-gray-800/50 text-left border border-transparent hover:border-gray-700 transition-colors">
                        <div className="w-10 h-10 rounded bg-gray-800 flex items-center justify-center shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-gray-500 truncate mt-0.5">{buildLocationPathString(loc.id, locations)}</div>
                        </div>
                        <div className="text-right shrink-0">
                          <div className="text-sm text-white font-semibold">{partCount}</div>
                          <div className="text-[10px] text-gray-500">parts</div>
                        </div>
                      </button>
                    );
          ...
          /* Normal browse layout: Tree | Content */
          <div className="flex-1 flex flex-col md:flex-row md:overflow-hidden">
            {/* Left Pane — Browse */}
            <div className="w-full md:w-[28%] lg:w-[24%] flex flex-col border-b md:border-b-0 md:border-r border-red-900/30 bg-black/20 max-h-[40vh] md:max-h-none">
              {!searchTerm && (
              ...
              <div className="flex-1 overflow-y-auto py-1">
                <div className={cn("flex items-center gap-2 px-3 py-2.5 cursor-pointer transition-colors", selectedLocationId === null ? "bg-red-950/40 text-red-400" : "hover:bg-gray-800/50 text-gray-300")} onClick={() => { setSelectedLocationId(null); setSelectedContainer(null); }}>
                  <div className="w-4" />
                ...
                {(showEmptyLocations || unassignedCount > 0) && (
                  <div className={cn("flex items-center gap-2 px-3 py-2.5 cursor-pointer transition-colors", selectedLocationId === 'unassigned' ? "bg-red-950/40 text-yellow-400" : "hover:bg-gray-800/50 text-gray-300", unassignedCount === 0 && "opacity-50")} onClick={() => { setSelectedLocationId('unassigned'); setSelectedContainer(null); }}>
                    <div className="w-4" />
            ...
            {/* Right Pane — What's here */}
            <div className="flex-1 flex flex-col overflow-hidden">
              {/* Location Detail — compact header */}
              {selectedLocationId && selectedLocationId !== 'unassigned' && !selectedContainer && (
              ...
              {/* Container Detail — takes over right pane */}
              {selectedContainer ? (
                    ...
                    {filteredParts.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-full text-center px-4">
                        ...
                        <h3 className="text-base font-medium text-gray-400 mb-1">
                          {selectedLocationId ? 'Nothing here' : 'No inventory'}
                        </h3>
                        ...
                        {groupedParts.map(group => (
                          <div key={group.locationId} className="space-y-3">
                            <div className="sticky top-0 z-10 flex items-center gap-2 px-3 py-2 bg-gray-900/95 backdrop-blur-sm rounded-lg border-l-4" style={{ borderLeftColor: group.color }}>
                              ...
                              <span className="text-xs text-gray-400">({group.subLocations.reduce((sum, sub) => sum + sub.parts.length, 0)})</span>
                            </div>
                                ...
                                {viewMode === 'list' ? (
                                  <div className={cn("space-y-2", subLoc.locationName && "ml-4")}>
                                    {subLoc.parts.map(part => {
                                      ...
                                      return (
                                        <StoragePartRow key={`${part.id}-${subLoc.locationId}`} part={part}
                                          locationQty={part._locationQty} locationReserved={part._locationReserved}
                                          locationId={subLoc.locationId || (group.locationId === 'unassigned' ? 'unassigned' : null)}
                                          selectedLocationId={selectedLocationId} getInventoryStats={getInventoryStats}
                                          getInventoryItemId={getInventoryItemId} vendors={vendors}
                                          onPartClick={onPartClick} onOpenGallery={openGallery} partActions={partActions}
                                  ...
                                  <div className={cn("grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3", subLoc.locationName && "ml-4")}>
                                    {subLoc.parts.map(part => (
                                      <StoragePartCard key={`${part.id}-${subLoc.locationId}`} part={part}
                                        locationQty={part._locationQty} locationReserved={part._locationReserved}
                                        locationId={subLoc.locationId || (group.locationId === 'unassigned' ? 'unassigned' : null)}
                                        selectedLocationId={selectedLocationId} getInventoryStats={getInventoryStats}
                                        getInventoryItemId={getInventoryItemId} vendors={vendors}
                  ...
                  {totalPartsCount > 0 && (
                    <div className="border-t border-red-900/20 bg-gray-900/30 p-2 px-4">
                      <div className="text-xs text-gray-500">
                        {totalPartsCount} part{totalPartsCount !== 1 ? 's' : ''} · {groupedParts.length} location{groupedParts.length !== 1 ? 's' : ''}
                      </div>
                    </div>
                  )}
                </>
      ...
      {createContainerForLocation && (
        <CreateContainerModal onClose={() => setCreateContainerForLocation(null)} preselectedLocationId={createContainerForLocation} locations={locations} projects={projects} />
      )}
      {moveContainerTarget && (
        <MoveContainerModal container={moveContainerTarget} onClose={() => { setMoveContainerTarget(null); setMoveContainerReturnHome(false); }}
          locations={locations} inventoryItems={inventoryItems} returnHome={moveContainerReturnHome} />
      )}
      {addToContainerTarget && (
        <AddToContainerModal container={addToContainerTarget} onClose={() => setAddToContainerTarget(null)} inventoryItems={inventoryItems} parts={parts} />
      )}
      {emptyContainerTarget && (
        <EmptyContainerModal container={emptyContainerTarget} onClose={() => setEmptyContainerTarget(null)} locations={locations} containers={containers} inventoryItems={inventoryItems} parts={parts} />
      )}
