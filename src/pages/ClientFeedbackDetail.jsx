...
import { ArrowLeft, Send, Upload, Link as LinkIcon, Loader2, Archive, CheckCircle2, AlertCircle, Plus, ExternalLink, X, Paperclip, Trash2, RotateCw, FileText, Image as ImageIcon } from "lucide-react";
    ...
    <>
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black p-3 md:p-6">
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          {/* Side-by-side image comparison for Design Reviews */}
          {request.request_type === 'image_review' && (designImages.length > 0 || clientFeedbackImages.length > 0) && (
            <Card className="bg-black/40 backdrop-blur-xl border border-gray-700">
              <CardContent className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Design Images Column */}
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      <ImageIcon className="w-4 h-4 text-purple-400" />
                      <h3 className="font-semibold text-white text-sm">Design Images</h3>
                    ...
                    {designImages.length > 0 ? (
                      <div className="grid grid-cols-2 gap-2">
                        {designImages.map(att => (
                          ...
                          >
                            <div className="w-full h-32 bg-gray-800 rounded-lg border border-gray-700 overflow-hidden hover:border-purple-500/50 transition-colors">
                              <img 
                                src={att.file_url} 
                                alt="" 
                      ...
                      <div className="h-32 bg-gray-800/50 rounded-lg border border-dashed border-gray-700 flex items-center justify-center">
                        <p className="text-gray-500 text-sm">No design images</p>
                      </div>
                    )}
                  </div>

                  {/* Client Feedback Images Column */}
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      <Upload className="w-4 h-4 text-blue-400" />
                      <h3 className="font-semibold text-white text-sm">Client Feedback Images</h3>
                      <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/50 border text-xs">
                        {clientFeedbackImages.length}
                      </Badge>
                    </div>
                    {clientFeedbackImages.length > 0 ? (
                      <div className="grid grid-cols-2 gap-2">
                        {clientFeedbackImages.map(att => (
                          <div 
                            key={att.id} 
                            className="relative group cursor-pointer"
                            onClick={() => setSelectedImage(att.file_url)}
                          >
                            <div className="w-full h-32 bg-gray-800 rounded-lg border border-gray-700 overflow-hidden hover:border-blue-500/50 transition-colors">
                              <img 
                                src={att.file_url} 
                                alt="" 
                                className="w-full h-full object-cover"
                      ...
                      <div className="h-32 bg-gray-800/50 rounded-lg border border-dashed border-gray-700 flex items-center justify-center">
                        <p className="text-gray-500 text-sm">No client feedback images</p>
                      </div>
                    )}
                  </div>
                </div>
