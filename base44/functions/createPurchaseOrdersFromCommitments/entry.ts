...
Deno.serve(async (req) => {
  try {
    ...
    if (dry_run) {
      ...
      return Response.json({ ok: true, dry_run: true, preview: { vendor_groups: Object.entries(vg).map(([v, items]) => ({ vendor_id: v, vendor_name: items[0]?.vendor_name, commitment_count: items.length, total_qty: items.reduce((s, i) => s + i.qty_to_order, 0), estimated_cost: items.reduce((s, i) => s + i.qty_to_order * i.unit_cost, 0) })), total_orders_to_create: Object.keys(vg).length, total_line_items: eligible.length }, blocked, phase1_warnings: warnings.length ? warnings : undefined, summary: { eligible_count: eligible.length, blocked_count: blocked.length, order_count: Object.keys(vg).length } });
    }
